...
def upload(
    msg: Dict, public_key: bytes, encrypt_data: Optional[bool] = True
) -> Tuple[str, str]:
    ...
    # If encryption is off, we'll use a public bucket
    is_bucket_public = encrypt_data is False
    bucket_name = get_bucket(public=is_bucket_public)

    bucket_kwargs = {"Key": key, "Bucket": bucket_name, "Body": content}
    ...
    if encrypt_data is True:
        ...
        bucket_kwargs.update({"ACL": "public-read"})
