...
import {
  IJobDto,
  IManifestDataItemDto,
  IManifestDto,
  jobFormatter,
  manifestFormatter,
} from "./serializers/job.responses";
...
export class JobService {
  ...
  constructor(
    ...
    this.recordingOracleAddress = this.configService.get<string>("RECORDING_ORACLE_ADDRESS", "");
    this.reputationOracleAddress = this.configService.get<string>("REPUTATION_ORACLE_ADDRESS", "");
    this.exchangeOracleAddress = this.configService.get<string>("EXCHANGE_ORACLE_ADDRESS", "");
  ...
  public async create(dto: IJobCreateDto, userId: number): Promise<IJobDto> {
    ...
    if (groundTruthFileUrl) {
      ...
      if (!this.isGroundTruthDataValid(groundTruthData)) {
    ...
    await this.web3.eth.getTransaction(dto.transactionHash);
    ...
    */
    ...
    await this.approve(jobEntity.id);
  ...
  public async createEscrow(jobEntity: JobEntity): Promise<string> {
    ...
    const escrowAddress: string = await escrowFactoryContract.methods
      .createEscrow([
    ...
    if (!escrowAddress) {
      throw new NotFoundException("Escrow was not created");
    }
  ...
  public async setupEscrow(jobEntity: JobEntity): Promise<string> {
    ...
    const escrowContract = new this.web3.eth.Contract(EscrowAbi as any, jobEntity.escrowAddress);

    const jobLauncherAddress = this.configService.get<string>("WEB3_JOB_LAUNCHER_ADDRESS", "");
    const jobLauncherUrl = this.configService.get<string>("JOB_LAUNCHER_URL", "");
    const manifestUrl = `${jobLauncherUrl}/job/${jobEntity.id}/manifest`;

    const result: string = await escrowContract.methods
      .setup(
        this.web3.utils.toChecksumAddress(jobEntity.reputationOracleAddress),
        this.web3.utils.toChecksumAddress(jobEntity.recordingOracleAddress),
      ...
      .send(
        { from: this.web3.utils.toChecksumAddress(jobLauncherAddress) },
        async (error: any, transactionHash: any) => {
          if (error) {
