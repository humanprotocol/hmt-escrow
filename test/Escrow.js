...
contract('Escrow', (accounts) => {
  ...
  it('sets parameters correctly when calling setup', async () => {
    await HMT.transfer(Escrow.address, 100, { from: accounts[0] })
    await Escrow.setup(reputationOracle, recordingOracle, 1, 1, 10, url, hash, { from: accounts[0] })
    const _reputationOracle = await Escrow.getReputationOracle.call();
    const _recordingOracle = await Escrow.getRecordingOracle.call();
    const _url = await Escrow.getUrl.call();
    const _hash = await Escrow.getHash.call();

    assert.equal(_reputationOracle, reputationOracle)
    assert.equal(_recordingOracle, recordingOracle)
  ...
  it('can store results when status is pending', async () => {
    try {
      await Escrow.setup(reputationOracle, recordingOracle, 1, 1, 0, url, hash, { from: accounts[0] })
      await Escrow.storeResults(url, hash, { from: accounts[2] });
      const _url = await Escrow.getIUrl.call();
      const _hash = await Escrow.getIHash.call();
      assert.equal(_url, url);
      assert.equal(_hash, hash);
    } catch (ex) {
      assert(false);
