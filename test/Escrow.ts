...
describe('Escrow', function() {
  ...
  beforeEach(async () => {
    ...
    escrow = await Escrow.deploy(token.address, await owner.getAddress(), 5, trustedHandlers);
  });
 ...
 describe('abort', () => {
    describe("Validations", function () {
      beforeEach(async () => {
        await escrow.connect(owner).setup(await reputationOracle.getAddress(), await recordingOracle.getAddress(), 10, 10, MOCK_URL, MOCK_HASH);
        await escrow.connect(owner).addTrustedHandlers([await reputationOracle.getAddress()]);
      });
      ...
      it("Should succeeds when aborting with not trusted address", async function () {
        ...
        await expect(escrow.connect(externalAddress).abort()).to.be.revertedWith("Address calling not trusted");
      });
    });
    ...
    describe("Calling abort", function () {
      ...
      it("Should transfer tokens to owner if contract funded when abort is called", async function () {
        ...
        expect(result.toString()).to.equal("0", "Escrow has not been properly aborted");
      });
    });
  });
  ...
  describe('addTrustedHandlers', async () => {
    ...
    describe("Validations", function () {
      it("Should revert with the right error if caller cannot add trusted handlers", async function () {
        await expect(escrow.connect(externalAddress).addTrustedHandlers([await reputationOracle.getAddress()])).to.be.revertedWith(
    ...
    describe("Add trusted handlers", async function () {
      it('Should succeeds when the contract launcher address trusted handlers and a trusted handler stores results', async () => {
        await (await escrow.connect(owner).addTrustedHandlers([await reputationOracle.getAddress()])).wait();

        const result: any = await (await escrow.connect(reputationOracle).storeResults(MOCK_URL, MOCK_HASH)).wait();
        
        const event = result.events[0].args
        expect(result.events[0].event).to.equal("IntermediateStorage", "IntermediateStorage event was not emitted")
  ...
  describe('storeResults', async () => {
    describe("Validations", function () {
      it("Should revert with the right error if address calling not trusted", async function () {
        await expect(
          escrow.connect(externalAddress).storeResults(
            MOCK_URL,
            MOCK_HASH
          )
        ).to.be.revertedWith(
      ...
      it("Should revert with the right error if escrow not in Pending or Partial status state", async function () {
        await escrow.connect(owner).addTrustedHandlers([await reputationOracle.getAddress()]);
        await expect(escrow.connect(reputationOracle).storeResults(MOCK_URL, MOCK_HASH)).to.be.revertedWith(
    ...
    describe("Events", function () {
      ...
      it("Should emit an event on intermediate storage", async function () {
        await expect(await escrow.connect(owner).storeResults(MOCK_URL, MOCK_HASH))
  ...
  describe('setup', () => {
    describe("Validations", function () {
      it("Should revert with the right error if address calling not trusted", async function () {
        await expect(
          escrow.connect(externalAddress).setup(
    ...
    describe("Setup escrow", async function () {
      ...
      it('Should sets correct escrow with params', async () => {
        await escrow.connect(owner).setup(
          await reputationOracle.getAddress(),
          await recordingOracle.getAddress(),
          10,
          10,
        ...
        expect(await escrow.reputationOracle()).to.equal(await reputationOracle.getAddress())
        expect(await escrow.recordingOracle()).to.equal(await recordingOracle.getAddress())
  ...
  describe('cancel', () => {
    describe("Validations", function () {
      beforeEach(async () => {
        ...
        await escrow.connect(owner).bulkPayOut([await restAccounts[0].getAddress()], [100], MOCK_URL, MOCK_HASH, '000')
      });
  ...
  describe('bulkPayOut', () => {
    describe("Validations", function () {
      ...
      it("Should revert with the right error if address calling is not trusted", async function () {
        ...
        await expect(
          escrow.connect(externalAddress).bulkPayOut(recepients, amounts, MOCK_URL, MOCK_HASH, '000')
        ).to.be.revertedWith(
          "Address calling not trusted"
        );
      });
      ...
      it("Should revert with the right error if amount of recipients more then amount of values", async function () {
        const recepients = [await restAccounts[0].getAddress(), await restAccounts[1].getAddress(), await restAccounts[2].getAddress()]
        const amounts = [10, 20]

        await expect(
          escrow.connect(reputationOracle).bulkPayOut(recepients, amounts, MOCK_URL, MOCK_HASH, '000')
        ).to.be.revertedWith(
      ...
      it("Should revert with the right error if amount of recipients less then amount of values", async function () {
        const recepients = [await restAccounts[0].getAddress(), await restAccounts[1].getAddress()]
      ...
      it("Should revert with the right error if too many recipients", async function () {
        const recepients = Array.from(new Array(BULK_MAX_COUNT + 1), () => ethers.constants.AddressZero);
    ...
    describe("Events", function () {
      ...
      it("Should emit an event on bulk transfer", async function () {
        ...
        await expect(
          escrow.connect(owner).bulkPayOut(recepients, amounts, MOCK_URL, MOCK_HASH, '000')
        )
    ...
    describe("Bulk payout for recipients", async function () {
      ...
      it('Should pays each recipient their corresponding amount', async () => {
        ...
        const initialBalanceAccount1 = await token.connect(owner).balanceOf(account1);
        const initialBalanceAccount2 = await token.connect(owner).balanceOf(account2);
        const initialBalanceAccount3 = await token.connect(owner).balanceOf(account3);
        const initialBalanceRecordingOracle = await token.connect(owner).balanceOf(await recordingOracle.getAddress());
        const initialBalanceReputationOracle = await token.connect(owner).balanceOf(await reputationOracle.getAddress());
        ...
        await escrow.connect(reputationOracle).bulkPayOut(recepients, amounts, MOCK_URL, MOCK_HASH, '000')

        const finalBalanceAccount1 = await token.connect(owner).balanceOf(account1);
        const finalBalanceAccount2 = await token.connect(owner).balanceOf(account2);
        const finalBalanceAccount3 = await token.connect(owner).balanceOf(account3);
        const finalBalanceRecordingOracle = await token.connect(owner).balanceOf(await recordingOracle.getAddress());
        const finalBalanceReputationOracle = await token.connect(owner).balanceOf(await reputationOracle.getAddress());

        expect((finalBalanceAccount1.toNumber() - initialBalanceAccount1.toNumber()).toString()).to.equal("8");
        expect((finalBalanceAccount2.toNumber() - initialBalanceAccount2.toNumber()).toString()).to.equal("16");
        expect((finalBalanceAccount3.toNumber() - initialBalanceAccount3.toNumber()).toString()).to.equal("24");
        expect((finalBalanceRecordingOracle.toNumber() - initialBalanceRecordingOracle.toNumber()).toString()).to.equal("6");
        expect((finalBalanceReputationOracle.toNumber() - initialBalanceReputationOracle.toNumber()).toString()).to.equal("6");
