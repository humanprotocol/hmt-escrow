...
class StorageTest(unittest.TestCase):
    ...
    def test_upload_to_private_bucket(self):
        ...
        with patch('hmt_escrow.storage._connect_s3') as mock_s3:
            ...
            upload(self.serialized_manifest, self.pub_key, encrypt_data=True)

        mock_s3.assert_called()

        self.assertIn('Bucket', s3_client_mock.put_object.call_args.kwargs.keys())

        # With encryption on, bucket is meant to be the private one
    ...
    def test_upload_to_public_bucket(self):
        """ Tests uploading file to storage to public bucket. """
        ...
        with patch('hmt_escrow.storage._connect_s3') as mock_s3:
            ...
            upload(self.serialized_manifest, self.pub_key, encrypt_data=False)
        ...
        self.assertEqual(s3_client_mock.put_object.call_args.kwargs['Bucket'], ESCROW_TEST_PUBLIC_BUCKETNAME)

    def test_upload(self):
        """ Tests uploading file to storage with encryption on/off. """
        manifest_to_string = json.dumps(self.serialized_manifest, sort_keys=True)
        hash_ = hashlib.sha1(manifest_to_string.encode('utf-8')).hexdigest()
        key = f"s3{hash_}"

        # Upload with encryption on (default)
        generated_hash, manifest_url = upload(self.serialized_manifest, self.pub_key)
        self.assertEqual(hash_, generated_hash)
        self.assertEqual(key, manifest_url)

        # Upload with encryption off
        generated_hash, manifest_url = upload(self.serialized_manifest, self.pub_key, encrypt_data=False)
        self.assertEqual(hash_, generated_hash)
        self.assertEqual(key, manifest_url)
    ...
    def test_download_from_storage_from_private_bucket(self):
        ...
        # With encryption on, bucket is meant to be the public one
        self.assertEqual(s3_client_mock.get_object.call_args.kwargs['Bucket'], ESCROW_TEST_BUCKETNAME)

    @patch("hmt_escrow.storage.ESCROW_PUBLIC_BUCKETNAME", ESCROW_TEST_PUBLIC_BUCKETNAME)
    def test_download_from_storage_from_public_bucket(self):
        """ Tests download of file artifact from storage from private bucket. """
        s3_client_mock = MagicMock()
        with patch('hmt_escrow.storage._connect_s3') as mock_s3:
            mock_s3.return_value = s3_client_mock
        ...
        # With encryption on, bucket is meant to be the public one
        self.assertEqual(s3_client_mock.get_object.call_args.kwargs['Bucket'], ESCROW_TEST_PUBLIC_BUCKETNAME)

    def test_download_from_storage(self):
        """ Tests download from storage with encryption on/off"""

        # Upload with encryption on (default)
        _, manifest_url = upload(self.serialized_manifest, self.pub_key)
        ...
        self.assertEqual(json.loads(decrypted), self.serialized_manifest)

        # Upload with encryption off
        _, manifest_url = upload(self.serialized_manifest, self.pub_key, encrypt_data=False)
        # Encryption off determines that data is public
        content = download_from_storage(key=manifest_url, public=True)

        # As encryption is off, data is plain
        self.assertEqual(json.loads(content), self.serialized_manifest)
    ...
    def test_upload_with_encryption_option(self):
        ...
        _, manifest_url = upload(self.serialized_manifest, self.pub_key, encrypt_data=True)
        data = download_from_storage(manifest_url, public=False)
        self.assertEqual(crypto.is_encrypted(data), True)

        # Uplaoding data flagging to send it encrypted.
        _, manifest_url = upload(self.serialized_manifest, self.pub_key, encrypt_data=False)
        data = download_from_storage(manifest_url, public=True)
    ...
    def test_public_private_download_from_storage(self):
        ...
        with patch('hmt_escrow.storage.download_from_storage') as download_mock:
            ...
            download_mock.side_effect = [crypto.encrypt(self.pub_key, sample_data), sample_data.encode('utf-8')]
    ...
    def test_download(self):
        """ Tests download of the correct artifact persisted in storage with encryption on/off. """
        _, manifest_url = upload(self.serialized_manifest, self.pub_key)

        manifest_dict = download(manifest_url, self.priv_key)
        self.assertEqual(manifest_dict, self.serialized_manifest)
